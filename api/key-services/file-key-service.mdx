...
```typescript
import { createFileKeyService } from 'near-api-ts';
import { parsePrivateKey } from 'near-api-ts/crypto';
...
const keyPair = parsePrivateKey(privateKeyString);

await keyService.addKeyPair(keyPair);
console.log('Imported key:', keyPair.publicKey);
```
